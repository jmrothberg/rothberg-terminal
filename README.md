...
**Release:** the live header and browser tab show **v1.5.13** so you can confirm you are on the current build after a refresh or deploy.
...
#### EXTERNAL ACCOUNT — what “Fidelity CSV format” means

The importer expects the **positions export layout Fidelity produces**, not an arbitrary CSV:

- **Where to get it:** Fidelity → **Accounts** → **Positions** (or Portfolio) → **Download** / export → save the positions file (often `Portfolio_Positions_<date>.csv`). Use **ADD CSV** in the EXTERNAL ACCOUNT tray.
- **Headers (case-insensitive):** **`Account Name`** and **`Symbol`** are required for rows to import. Optional columns used when present: `Account Number`, `Description`, `Quantity`, `Last Price`, `Current Value`, `Cost Basis Total`, `Average Cost Basis`, `Today's Gain/Loss Dollar`, `Total Gain/Loss Dollar`, `Type`. Money/quantity parsing accepts Fidelity-style `$`, commas, and parentheses for negatives; quoted fields are supported.
- **Other brokers:** There is no generic column mapper. To use this panel, supply a CSV with **these same header names** and one row per position line (or export from Fidelity). Reshape another broker’s export in a spreadsheet to match, then save as CSV.

The in-app **[HELP]** panel has the same summary under **EXTERNAL ACCOUNT — FIDELITY CSV FORMAT**.

### Fifteen panel types — reassign any panel to any type
...
| **FLIGHTS** | ADS-B aircraft within a configurable radius of a pinned lat/lon | **CENTER** — type `lat, lon` + Enter to re-pin (defaults to a Miami Beach address). **RADIUS** chips: `5 / 10 / 25 / 50 nm`. **DISPLAY → MAP** = top-down 2D map: home as a ★ at center, range rings, aircraft as triangles rotated to heading (color by altitude band: amber < 5k ft, bright green < 18k ft, dim green ≥ 18k ft), fading position trails, hover for tooltip, click to pin. **DISPLAY → LIST** = sortable rows (closest first) with callsign · operator · plane type · origin → destination · altitude · speed · distance from home. Aircraft come from [adsb.lol](https://adsb.lol/) (CORS-open, no key); origin / destination / airline come from [adsbdb.com](https://www.adsbdb.com/) per callsign (cached 24h). |
...
- **TROPICAL CYCLONES** — NOAA National Hurricane Center active-storms feed. Basin chips (`All` / `Atlantic` / `East Pacific` / `Central Pacific`); multi-select matches storms in **any** selected basin. Each row shows the Saffir-Simpson category derived from 1-minute sustained wind (or `TD` / `TS` for sub-hurricane systems), wind speed in mph, central pressure in mb, and links to the NHC public advisory.
