...
**Release:** the live header and browser tab show **v1.6.4** so you can confirm you are on the current build after a refresh or deploy.
...
| Flights (address geocoding) | [OpenStreetMap Nominatim](https://nominatim.openstreetmap.org/) `search?q=&format=json` | Proxy rotation | CORS-open; only called when the user types a non-numeric **CENTER** input. Asks for ≤ 1 req/sec — the panel only fires one per CENTER edit, so we're well under |
